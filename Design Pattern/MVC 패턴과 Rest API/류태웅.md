...
* Spring 구현 방안
  ...
  * @RestController
    ...
    * @RestController vs @Controller
      * 기존 Spring MVC구조의 @Controller는 컨트롤러의 리턴값이 ViewResolver에 의해서 해석되어서 **특정뷰가 html응답으로 전송**된다. (즉 View를 반환한다.)
      * 반면 @RestController에 의한 호출은 뷰를 리턴하는 것이 목적이 아닌, **데이터를 전송하는 목적으로 응답이 전송**되어 진다. (즉 JSON을 반환한다.)
